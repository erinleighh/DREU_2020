...
def findsd(relFlux, time):
    ...
    return maximumSD, avgMaximumSD, potentialEclipses[maxSDRangeIndex] * -1,  peTimes.iloc[maxSDRangeIndex], relFlux.iloc[maxSDRangeIndex]
...
for file in files:
    ...
    try:
        ...
        AOK = (curveTable['QUALITY'] == 0) & ((curveTable['TIME'] < 1347) | (curveTable['TIME'] > 1350))
        curveData = curveTable.loc[AOK].dropna(subset=['TIME']).dropna(subset=['PDCSAP_FLUX']).copy()
        ...
        while classif == 'notEB' and i < 3:  # Potential to be an EB.
            ...
            if avgMaxSD < 3 or min(rangeSD) < 1:
                # Pre-classification
                ...
                try:
                    ...
                    lcTable = addtotable(lcTable, objName, '', '', maxSD, avgMaxSD, i, flag, classif, rangeSD, file)
                    pass
                break
            else:
